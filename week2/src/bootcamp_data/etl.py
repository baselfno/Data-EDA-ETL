...
def transform(orders_raw: pd.DataFrame, users: pd.DataFrame) -> pd.DataFrame:
    ...
    joined = joined.assign(amount_winsor=joined["amount"])
    joined = winsorize(joined, "amount_winsor")
    joined = add_outlier_flag(joined, "amount_winsor", k=1.5)
